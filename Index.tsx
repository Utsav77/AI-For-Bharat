...
export default function ShramSetuSaathi() {
  ...
  const audioRef = useRef<HTMLAudioElement | null>(null); // holds the TTS Audio instance
  ...
  const callAPI = useCallback(async (query: string, asrMs?: number) => {
    ...
    try {
      ...
      // ── 5. TTS audio URL — prime inside gesture context, play when results appear ──
      ...
      if (audioUrl) {
        ...
        // CRITICAL: play→pause immediately while still inside the user gesture call
        // stack. This unlocks the element so the deferred .play() in the results
        // useEffect succeeds without requiring a new user interaction.
        audio.play().then(() => {
          audio.pause();
          audio.currentTime = 0;
        }).catch(() => {
          // Some browsers reject even this — results useEffect has its own fallback
        });
      }
